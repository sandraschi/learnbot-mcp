    ...
    async with get_db() as db:
        cur = await db.execute(
            """INSERT INTO lessons (title, description, language, level, author, sections, vocab, quiz, duration_min, tags)
               VALUES (?,?,?,?,?,?,?,?,?,?)""",  # noqa: E501
            (
                title, description, language, level, author,
                json.dumps(sections or []), json.dumps(vocab or []),
                json.dumps(quiz or []), duration_min, json.dumps(tags or []),
            ),
        )
        await db.commit()
...
async def lesson_update(lesson_id: int, **kwargs) -> dict:
    ...
    await ensure_lessons_table()
    allowed = {"title", "description", "language", "level", "author", "sections", "vocab", "quiz", "duration_min", "tags"}  # noqa: E501
    sets = []
    params: list[Any] = []
...
async def lesson_generate(title: str, language: str = "ja", level: str = "N4", duration_min: int = 15) -> dict:  # noqa: E501
    ...
    try:
        ...
        return await lesson_create(
            ...
            sections=data.get("sections", []), vocab=data.get("vocab", []),
            quiz=data.get("quiz", []), duration_min=duration_min,
            tags=data.get("tags", []),
        )
...
async def lesson_run(lesson_id: int, conversation_id: str) -> dict:
    ...
    async with _db() as db:
        ...
        await db.execute("UPDATE conversations SET turn_count=turn_count+1, updated_at=? WHERE id=?", (stamp, conversation_id))  # noqa: E501
        await db.commit()
    # Build a lesson prompt for the LLM
    sections_text = "\n\n".join(
        f"[{s.get('type', 'section').upper()}] {s['content']}"
    ...
    quiz_text = "\n".join(
        f"  Q: {q.get('question', '?')}" for q in lesson.get("quiz", [])
    ) if lesson.get("quiz") else "No quiz."
    lesson_prompt = (
