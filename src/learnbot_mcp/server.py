...
async def chat_send(
    ...
    # If a lesson is active for this conversation (via lesson_run), fold its
    # prompt in so the LLM actually teaches it instead of lesson_run's output
    # being a dead end.
    ...
    if conv_metadata:
        try:
            meta = _json.loads(conv_metadata)
        except (TypeError, ValueError):
            meta = {}
        lesson_prompt = meta.get("lesson_prompt")
        if lesson_prompt:
...
async def lesson_differentiate(
    lesson_id: int, target_level: str, language: str = "ja", duration_min: int | None = None
) -> dict:
    """Adapt an existing lesson for a different JLPT level and save as new lesson.

    The original lesson is unchanged. The adapted version gets a new title
    like "Original (N5)" and is created as a separate entry.

    ## Return Format
    {"success": bool, "lesson": dict,
     "source_lesson_id": int, "source_level": str,
     "target_level": str}
    """
    from learnbot_mcp.lessons import lesson_differentiate as _ld

    return await _ld(
        lesson_id=lesson_id, target_level=target_level, language=language, duration_min=duration_min
    )
